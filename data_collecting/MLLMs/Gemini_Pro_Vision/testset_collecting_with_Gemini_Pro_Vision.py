...
filename = 'data/MLLMs/Gemini_Pro_Vision/triplet_dataset/testset_Gemini_Pro_Vision.txt'
if not os.path.exists(filename):
    with open(filename, 'w') as file:
        pass
line_count = count_lines(filename)

testset_humans = 'data/Humans/triplet_dataset/testset1.txt' # or testset2.txt, testset3.txt, testset2_repeat.txt
total_lines = count_lines(testset_humans)
...
for trial in range(start_trial, end_trial):
    ...
    write_trial_data_to_csv('data/MLLMs/Gemini_Pro_Vision/triplet_dataset/prompts&responses_testset_Gemini_Pro_Vision.csv', group_results,
                            group_prompts, group_responses)

    print("model:", sorted_images)
    # Save the sorted image numbers to a txt file
    with open('data/MLLMs/Gemini_Pro_Vision/triplet_dataset/testset_Gemini_Pro_Vision.txt', 'a') as f:
