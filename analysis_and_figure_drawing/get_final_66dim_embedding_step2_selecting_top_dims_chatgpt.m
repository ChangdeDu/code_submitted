...
spose_embedding_61d_sorted = spose_embedding_topd(:, sorted_indices);

% manual inspection found distinct uniqueness in these dimensions, thus add
% them
top_num = 70;
[sorted_values, sorted_indices] = sort(mean_reproducibility, 'descend');
top_indices = sorted_indices(1:top_num);
...
column_sums = sum(spose_embedding_topd);
[~, sorted_indices] = sort(column_sums, 'descend');
spose_embedding_topd_sorted_temp = spose_embedding_topd(:, sorted_indices);
add_index= [35,56,59,63,66]; 
spose_embedding_5d_sorted_add = spose_embedding_topd_sorted_temp(:,add_index);

spose_embedding_66d_sorted = [spose_embedding_61d_sorted spose_embedding_5d_sorted_add];
filename='data/LLMs/ChatGPT-3.5/spose_embedding_66d_sorted_chatgpt.txt';
