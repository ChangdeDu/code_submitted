...
load(fullfile(data_dir,'dimlabel_answers_gemini.mat'));
load(fullfile(variable_dir,'im.mat'))

%% get dimension labels, short labels and colors
labels_short = importdata(fullfile(variable_dir,'labels_short_66_gemini.txt'));
...
for i_dim = 1:66
    ...
    for i_sub = 1:10
        s = strsplit(dimlabel_answers_gemini{i_sub,i_dim},',');
        for k = 1:length(s)
            s{k} = strtrim(s{k});
        end
...
n_im = 6;
tempFolder = 'temp_figures_gemini';
...
for i_dim = [1:66] % chosen dimensions
    ...
    figFile = fullfile(tempFolder, sprintf('figure_%d_gemini.pdf', i_dim));
