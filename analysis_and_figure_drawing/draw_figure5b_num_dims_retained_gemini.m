...
load(fullfile(data_dir,'gemini_spose_similarity.mat'))
% spose_sim=embedding2sim(spose_embedding66);
dissim = 1-spose_sim;
% load test set
triplet_testdata66 = load(fullfile(data_dir,'triplet_dataset\validationset_Gemini_Pro_Vision.txt'))+1; % 0 index -> 1 index
...
fn1 = fullfile(data_dir,'gemini_spose_similarity_reduced.mat');
fn2 = fullfile(data_dir,'gemini_spose_embedding66_reduced.mat');
...
if ~exist(fn1,'file') || ~exist(fn2','file')
    ...
    for i_dim = 1:66
        ...
        if i_dim == 1
            gemini_spose_embedding66_reduced{i_dim,1} = spose_embedding66;
        else
            gemini_spose_embedding66_reduced{i_dim,1} = gemini_spose_embedding66_reduced{i_dim-1,1};
        end
        for i = 1:1854
            gemini_spose_embedding66_reduced{i_dim,1}(i,embedding_sortind(i,i_dim)) = 0;
        end
        gemini_spose_similarity_reduced{i_dim,1} = embedding2sim(gemini_spose_embedding66_reduced{i_dim,1});
        
    end
    fprintf('\n')
    save(fn1,'gemini_spose_similarity_reduced')
    save(fn2,'gemini_spose_embedding66_reduced')
else
    load(fn1)
    load(fn2)
end
...
for i_dim = 1:66
    rng(42) % for reproducibility
    behav_predict = zeros(length(triplet_testdata66),1);
    dot_product66_reduc = gemini_spose_embedding66_reduced{i_dim}*gemini_spose_embedding66_reduced{i_dim}';
    for i = 1:length(triplet_testdata66)
...
for i_dim = 1:66
    r_reduc(i_dim) = corr(squareformq(spose_sim),squareformq(gemini_spose_similarity_reduced{i_dim}));
end
...
title('Prediction of MLLM behavior', 'FontSize', 16)
ax = gca;
set(ax, 'FontSize', 10);

if dosave
    exportgraphics(fig, 'acc_vs_dims_retained_gemini.pdf', 'ContentType', 'vector');
end
...
if dosave
    exportgraphics(fig, 'var_vs_dims_retained_gemini.pdf', 'ContentType', 'vector');
end
